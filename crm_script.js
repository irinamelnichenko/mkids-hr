...
// ── ДІАГНОСТИКА: класифікація рядків Salary ────────────────────────────────
// auditSalaryClassification() — для Кар'єрна / Борщагівка / Бровари виводить
// усі непорожні рядки колонки A Salary-файлу + каталоги предметників і
// додаткових занять. Лише читання. Запускати ВРУЧНУ з Apps Script editor.
function auditSalaryClassification(){
  var LOCS = ["Кар'єрна", 'Борщагівка', 'Бровари'];
  var reg = _salaryGetRegistry();
  if (!reg.ok){ Logger.log('Salary-реєстр: %s', reg.error); return reg; }
  var regByLoc = {};
  reg.rows.forEach(function(r){ regByLoc[r.loc] = r; });
  ...
  LOCS.forEach(function(loc){
    Logger.log('\n════════════════ %s ════════════════', loc);
    var entry = regByLoc[loc];
    if (!entry){ Logger.log('  ❌ локацію не знайдено у Salary-реєстрі CONFIG'); return; }
    var sheet;
    try { sheet = SpreadsheetApp.openById(entry.sheetId).getSheetByName(entry.listName); }
    catch(e){ Logger.log('  ❌ доступ до Salary: %s', e && e.message || e); return; }
    if (!sheet){ Logger.log('  ❌ лист "%s" не знайдено', entry.listName); return; }

    var lastRow = sheet.getLastRow();
    if (lastRow >= 4){
      var colA = sheet.getRange(4, 1, lastRow - 3, 1).getValues();
      var shown = 0;
      for (var i = 0; i < colA.length; i++){
        var raw = String(colA[i][0] == null ? '' : colA[i][0]).trim();
        if (!raw) continue;
        Logger.log('[%s] A%s "%s"', loc, i + 4, raw);
        shown++;
      }
      Logger.log('  (непорожніх рядків колонки A: %s)', shown);
    } else {
      Logger.log('  (лист порожній — lastRow=%s)', lastRow);
    }

    var cat = getPredmetnyCatalog(loc);
    var catSubj = ((cat && cat.items) || []).map(function(a){ return a.subject; });
    Logger.log('[%s] Каталог предметників: %s', loc, catSubj.join(', ') || '—');

    var act = getActivitiesCatalog(loc);
    var actNames = ((act && act.items) || []).map(function(a){ return a.name; });
    Logger.log('[%s] Каталог додаткових: %s', loc, actNames.join(', ') || '—');
  });
  return {ok: true};
}
