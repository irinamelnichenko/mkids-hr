...
| | |
|---|---|
| Версія у репозиторії | **v7.163** |
| md5 | `5ff04dea79ec96d6a7318c4cfc5e0c36` |
| Розмір | 1351787 байт |
| Зафіксовано | 2026-08-19 |
| Версія у проді | **v7.160** — v7.163 ЩЕ НЕ ЗАДЕПЛОЄНО |
| Перевірка | `action=ping` → має стати `pong v7.163` після деплою |
...
Оригінали зачеплених рядків лягають в аркуш `BACKUP_Табель_ремап_<stamp>`
з колонкою `_рядок_до_ремапу` — саме він, а не lastRow, є точкою відкату,
бо ремап ПРАВИТЬ і ВИДАЛЯЄ рядки, а не тільки дописує.
